...
static void location_class_create(std::map<std::string, std::string> &attributes, std::map<int, std::string> error_map, LocationClass *location)
{
...
void location_add(std::list<std::list<std::string> > &lines, std::list<std::list<std::string> >::iterator &it, LocationClass *new_loc)
{
	...
	while (it != lines.end() && *(((*it).begin())) != "}")
	{
		if ((it_map = attributes.find(*((*it).begin()))) != attributes.end())
		{
			...
			else if (*((*it).begin()) == "error_page" && (*it).size() == 3)
			{
	...
	location_class_create(attributes, error_map, new_loc);
}
...
int server_add(std::list<std::list<std::string> > &lines, std::list<std::list<std::string> >::iterator &it, std::vector<ConfigClass*> &conf)
{
	...
	while (it != lines.end())
	{
		...
		{
			new_loc = new LocationClass();
			location_add(lines, it, new_loc);
